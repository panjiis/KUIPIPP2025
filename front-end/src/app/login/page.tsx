  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('http://localhost:5000/api/admin/login', {
        ...
        credentials: 'include', // Penting untuk menyimpan cookie session
      ...
      window.location.href = '/Admin'; // Ganti dengan URL dashboard Anda
        ...
        <form className="space-y-6" onSubmit={handleLogin}>
          ...
          <div>
            <button
              type="submit"
              disabled={loading}
              className="w-full flex justify-center items-center gap-2 py-3 px-4 border border-transparent rounded-lg shadow-sm text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:bg-neutral-600"
            >
              {loading ? (
