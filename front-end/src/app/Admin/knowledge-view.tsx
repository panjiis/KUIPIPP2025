      ...
      <div className="p-6 overflow-y-auto space-y-8 bg-white/5">
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
            <div className="text-xs font-medium text-gray-400 mb-[-8px]">Ketik Ini</div>
            <div className="text-xs font-medium text-gray-400 mb-[-8px]">Hasil</div>
    ...
    {isEditing ? (
      type === 'textarea' ? (
        <textarea
          ...
          className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-transparent transition-colors font-mono text-gray-100 placeholder-gray-400"
        />
      ) : (
        <input
          type={type}
          ...
          onChange={onChange}
          placeholder={placeholder}
          className="w-full bg-white/5 border border-white/10 rounded-lg p-3 text-sm focus:ring-2 focus:ring-primary/30 focus:border-transparent transition-colors text-gray-100 placeholder-gray-400"
        />
      )
    ) : (
      <div className="bg-white/5 p-4 rounded-lg text-sm leading-relaxed border border-[#13484f]/40 text-black overflow-x-auto backdrop-blur-sm">
        {useMarkdown ? (
          <Suspense fallback={<div className="text-sm text-[#13484f]">{value}</div>}>
...
export default function KnowledgeView({ onBack }: KnowledgeViewProps) {
  ...
  const handleToggleStatus = useCallback(
    async (id: string) => {
      try {
        ...
        toast.success(`Status "${updatedItem.topic}" diubah menjadi ${updatedItem.status}`);
      } catch (err) {
        toast.error(err instanceof Error ? err.message : 'Terjadi kesalahan.');
      }
    },
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 h-full flex flex-col">
      <header className="mb-8 flex justify-between items-start glass-card p-6 border border-white/10">
        <div>
          <h1 className="text-3xl font-bold text-[#13484f] tracking-tight">Knowledge Base</h1>
          <p className="text-[#13484f] mt-1">Manajemen dan monitoring Basis Pengetahuan chatbot.</p>
        </div>
        <button
          onClick={onBack}
      ...
      <section className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div className="glass-card p-6 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[#13484f]">Update RAG</h2>
            <p className="text-[#13484f] mt-1">Perbarui model dengan data terbaru (Wajib jika ada perubahan).</p>
          </div>
          <button
        ...
        <div className="glass-card p-6 flex items-center justify-between">
          <div>
            <h2 className="text-lg font-semibold text-[#13484f]">Tambah Informasi</h2>
            <p className="text-[#13484f] mt-1">Input item pengetahuan baru.</p>
          </div>
          <button
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10 flex-1 pb-3">
        <div className="lg:col-span-1 glass-card h-[710px] flex flex-col overflow-hidden">
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-3 rounded-lg transition-all border ${
                      selectedItem?._id === item._id ? 'bg-white/8 border-primary/40' : 'border-transparent hover:bg-white/6'
                    }`}
                  >
                    <div className="flex justify-between items-center mb-1">
                      <p className="font-bold text-[#13484f] text-sm truncate w-[70%]">{item.topic}</p>
                      <div className="flex items-center gap-1.5">
                        {item.is_sync ? <Cloud className="w-3.5 h-3.5 text-blue-400" /> : <CloudOff className="w-3.5 h-3.5 text-orange-400" />}
                    ...
                    <div className="flex items-center justify-between mt-1">
                      <p className="text-[10px] text-[#13484f]">{item.category}</p>
                      <p className="text-[10px] text-[#13484f]">{new Date(item.updatedAt).toLocaleDateString()}</p>
                    </div>
                  </button>
                );
              })
        ...
        <div className="lg:col-span-2 glass-card h-[710px] flex flex-col overflow-hidden">
        ...
        <header className="p-4 border-b border-white/10 flex justify-between items-center bg-white/5">
          <div>
            <h3 className="font-bold text-[#13484f]">
              {isAdding ? 'Tambah Informasi' : isEditing ? 'Edit Item' : item?.topic}
            </h3>
            {item && !isAdding && (
              <div className="flex items-center gap-2 mt-1">
                <p className="text-xs text-[#13484f] font-mono">ID: {item._id}</p>
              </div>
            )}
          </div>
                ...
                <>
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors flex items-center gap-1 ${
                      item.status === 'ACTIVE' ? 'bg-amber-100 text-amber-800' : 'bg-emerald-100 text-emerald-800'
                    }`}
                  >
                    {item.status === 'ACTIVE' ? (
                      <>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-white/5 space-y-4">
          {!isAdding && item && (
            <div className="flex flex-wrap gap-3 mb-4">
              ...
              <div
                className={`px-3 py-1.5 rounded-lg text-sm font-medium flex items-center gap-2 ${
                  item.is_sync ? 'bg-blue-50 text-blue-700' : 'bg-orange-50 text-orange-700'
                }`}
              >
                {item.is_sync ? <Cloud className="w-4 h-4" /> : <AlertCircle className="w-4 h-4" />}
                <span>Sync RAG: {item.is_sync ? 'Sudah (Synced)' : 'Belum (Tekan Update RAG)'}</span>
              </div>
            </div>
          )}
          ...
          <div className="mb-4">
            <label className="block text-sm font-medium text-[#13484f] mb-1">Kategori</label>
            {isAdding || isEditing ? (
              <CreatableSelect
                isClearable
                ...
                classNames={{
                  control: (state) =>
                    `!bg-white/5 !border-white/10 !rounded-lg !text-sm !shadow-none !p-1.5 ${
                      state.isFocused ? '!ring-2 !ring-blue-500 !border-transparent' : ''
                    }`,
                  menu: () => '!bg-white/5 !border !border-white/10 !rounded-lg !mt-1',
                  option: (state) =>
                    `!cursor-pointer !text-sm ${
                      state.isFocused ? '!bg-blue-50/30 !text-blue-200' : '!bg-white/5 !text-gray-100'
                    }`,
